    ...
    data: {
      ...
      players: {
        create: {
          ...
          displayName,
          avatarEmoji: data.hostAvatarEmoji,
          isHost: true,
          isReady: false,
...
export async function selectCategory(roomId: string, sessionToken: string, categoryId: string) {
  ...
  if (category.ageRating === '18+') {
    const hostUser = await prisma.user.findUnique({ where: { id: room.hostUserId } });
    if (!hostUser?.ageVerified18) throw new AppError('AGE_RESTRICTED', '18+ category requires age verification', 403);
  }
