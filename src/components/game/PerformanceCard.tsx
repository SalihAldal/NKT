...
export function PerformanceCard({ prompt, answerState, onComplete, onSkip }: PerformanceCardProps) {
  const locked = answerState === 'locked' || answerState === 'submitting' || answerState === 'timeout';

  return (
    <View style={styles.card}>
      <Text style={styles.badge}>🎭 PERFORMANS</Text>
      <ScrollView style={styles.promptScroll} nestedScrollEnabled>
        <Text style={styles.prompt}>{prompt}</Text>
      </ScrollView>
      {!locked ? (
        <View style={styles.actions}>
          <Button title="Canlandırmayı tamamladım ✓" onPress={() => onComplete?.()} />
...
const styles = StyleSheet.create({
  card: { backgroundColor: colors.card, borderRadius: 20, padding: spacing.xl, gap: spacing.lg, borderWidth: 1, borderColor: colors.accent + '44' },
  badge: { ...typography.small, color: colors.accent, fontWeight: '700' },
