...
import { calculateAgeYears } from '../auth/identity.js';
...
export async function writeAuditLog(data: {
  ...
  const requestId = data.requestId?.trim() || randomUUID();
...
export async function listUsers(params: { page: number; pageSize: number; search?: string; status?: string }) {
  ...
  const [users, total] = await Promise.all([
    prisma.user.findMany({
      ...
      include: {
        profile: true,
        identities: { where: { provider: 'USERNAME' }, take: 1 },
  ...
  const items = users.map((u) => ({
    ...
    age: u.birthDate ? calculateAgeYears(u.birthDate) : null,
...
export async function getUserDetail(userId: string) {
  ...
  return {
    ...
    age: user.birthDate ? calculateAgeYears(user.birthDate) : null,
